...
function useNumberFlowElementReady(): boolean {
  // Always starts false (server has no `customElements`, and client must
  // render identically on first hydration pass) — flips to true post-mount
  // once the custom element is confirmed registered. Evaluating
  // `customElements` directly in the useState initializer caused a
  // server/client hydration mismatch (same class of bug as branching on
  // `typeof window`), since the browser's real `customElements` registry
  // could already report "ready" by the time hydration ran.
